...
AVBufferRef* getVaapiContext(const torch::Device& device) {
  ...
  int err = av_hwdevice_ctx_create(
    &hw_device_ctx, type, renderD.c_str(), nullptr, 0);
  if (err < 0) {
    TORCH_CHECK(
        false,
...
vaapiSurface::vaapiSurface(VADisplay dpy, uint32_t width, uint32_t height):
  ...
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "Failed to create VAAPI surface: ",
    vaErrorStr(res));
}

void deleter(DLManagedTensor* self) {
  std::unique_ptr<DLManagedTensor> tensor(self);
  std::unique_ptr<ze_context_handle_t> context((ze_context_handle_t*)self->manager_ctx);
...
torch::Tensor vaapiSurface::toTensor(const torch::Device& device) {
  ...
  VAStatus sts = vaExportSurfaceHandle(
    dpy_, id_, VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
    VA_EXPORT_SURFACE_READ_ONLY, &desc);
  TORCH_CHECK(sts == VA_STATUS_SUCCESS, "vaExportSurfaceHandle failed: ",
      vaErrorStr(sts));

  // Sanity check returned description
  //TORCH_CHECK((desc.width == width) && (desc.height == height),
  //  "Expected ", width, "x", height, " surface, got", desc.width, "x", desc.height );
  TORCH_CHECK(desc.num_objects == 1,
    "Expected 1 fd, got ", desc.num_objects);
  TORCH_CHECK(desc.num_layers == 1,
    "Expected 1 layer, got ", desc.num_layers);
  ...
  queue.submit([&](sycl::handler &cgh) {
    cgh.host_task([&](const sycl::interop_handle &ih) {
      *ze_context = ih.get_native_context<sycl::backend::ext_oneapi_level_zero>();
      ze_device = ih.get_native_device<sycl::backend::ext_oneapi_level_zero>();
    });
  }).wait();
  ...
  //int64_t strides[3] = { va_desc.layers[0].pitch[0], 4, 1 };
...
VADisplay getVaDisplayFromAV(AVFrame* frame) {
...
struct vaapiVpContext {
  ...
  vaapiVpContext(
    VADisplay dpy, AVFrame* avFrame, uint16_t width, uint16_t height);
  ~vaapiVpContext() {
...
vaapiVpContext::vaapiVpContext(
  VADisplay dpy, AVFrame* avFrame,
  uint16_t width, uint16_t height):
  dpy_(dpy) {
  VAStatus res = vaCreateConfig(
    dpy_, VAProfileNone, VAEntrypointVideoProc, nullptr, 0, &config_id_);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "Failed to create VAAPI config: ",
    vaErrorStr(res));

  res = vaCreateContext(
    dpy_, config_id_, width, height, VA_PROGRESSIVE, nullptr, 0, &context_id_);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "Failed to create VAAPI VP context: ",
    vaErrorStr(res));
  ...
  //pipeline_.filter_flags = VA_FILTER_SCALING_FAST;

  res = vaCreateBuffer(
    dpy_, context_id_, VAProcPipelineParameterBufferType,
    sizeof(pipeline_), 1, &pipeline_, &pipeline_buf_id_);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "vaCreateBuffer failed: ",
    vaErrorStr(res));
}

void vaapiVpContext::convertTo(VASurfaceID id) {
  VAStatus res = vaBeginPicture(dpy_, context_id_, id);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "vaBeginPicture failed: ",
      vaErrorStr(res));

  res = vaRenderPicture(dpy_, context_id_, &pipeline_buf_id_, 1);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "vaRenderPicture failed: ",
      vaErrorStr(res));

  res = vaEndPicture(dpy_, context_id_);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "vaEndPicture failed: ",
      vaErrorStr(res));

  res = vaSyncSurface(dpy_, id);
  TORCH_CHECK(res == VA_STATUS_SUCCESS, "vaSyncSurface failed: ",
      vaErrorStr(res));
}
